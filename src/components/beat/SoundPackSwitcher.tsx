...
export function SoundPackSwitcher({ currentPackId, loading, onSwitch }: SoundPackSwitcherProps) {
  ...
  return (
    <div className="relative">
      <Tooltip content="Drum Kit — changes the sounds used by every track at once">
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
          aria-expanded={open}
        ...
        >
          <span className="text-ink-ghost text-[10px] font-normal">Kit:</span>
      ...
      {open && (
        <div
          role="listbox"
          aria-label="Drum kits"
          className="absolute right-0 top-full mt-1 z-50 w-72 rounded-xl border border-rim bg-panel shadow-xl overflow-hidden"
        >
          <div className="px-3 py-2.5 border-b border-rim">
            <p className="text-[11px] font-semibold text-ink">🥁 Choose a Drum Kit</p>
            <p className="text-[10px] text-ink-dim mt-0.5">
              Swaps the audio samples for <em>all</em> tracks at once. Your beat pattern stays the same — only the sounds change.
            </p>
          </div>
          ...
          <div className="px-4 py-2.5 border-t border-rim bg-well/50">
            <p className="text-[9px] text-ink-ghost leading-snug">
              💡 The dropdown on each track row lets you pick a <em>specific</em> drum sound (Kick, Snare, Hi‑Hat…) for that track only.
            </p>
          </div>
        </div>
      )}
